...
;   заданном 1-ым аргументом команды.
...
; выделяем память заданного размера. Размер задается как параметр команды при вызове.
...
stosd
mov eax, -1        ; fd == (-1), т.к. мы не связываем фай(дескриптор) с буфером
stosd
...
shr ecx, 2         ; делим на 4, получаем число элементов (1 элемент = 4 байта).
                   ; ecx - счетчик цикла

mov ebx, [buf]       ; записываем начало массива
mov eax, [counter]   ; задаем начальное значене счетчика циклов
;mov edx,0xABCDEF77
@@: ;Можно использовать stosd, edi. См. args_04.asm
  ...
  ; БУфер заполнили, осталость записать в файл.
;;;;;;;;;;;;; CREATE FILE ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
